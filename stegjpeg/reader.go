...
import (
	...
	"github.com/henkman/outguess/stegjpeg/internal/imageutil"
)
...
func (d *decoder) decode(r io.Reader, configOnly bool) (image.Image, Bitmap, error) {
	d.r = r
	d.bm.Bytes = 256
	d.bm.Bits = d.bm.Bytes * 8
	d.bm.Bitmap = make([]byte, d.bm.Bytes)
	...
	if d.tmp[0] != 0xff || d.tmp[1] != soiMarker {
		return nil, d.bm, FormatError("missing SOI marker")
	}
	...
	for {
		...
		if n < 0 {
			return nil, d.bm, FormatError("short segment length")
		}
	...
	if d.img3 != nil {
		...
		return d.img3, d.bm, nil
	}
	return nil, d.bm, FormatError("missing SOS marker")
}
...
// Decode reads a JPEG image from r and returns it as an image.Image.
func Decode(r io.Reader) (image.Image, Bitmap, error) {
	var d decoder
	img, bm, err := d.decode(r, false)
	bm.Bits = bm.off
	bm.Bytes = (bm.off + 7) / 8
	return img, bm, err
}
